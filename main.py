...
# Cell 2: Cargar el dataset
file_path = "dataset.csv"  # Cambiar la ruta si es necesario
...
# Cell 3: Información general del dataset
print("Columnas del dataset:")
print(df.columns)
print("\nDimensiones del dataset:")
print(df.shape)

# Cell 4: Eliminar columnas irrelevantes
df = df.drop('id', axis=1)  # Eliminar columna 'id'

# Cell 5: Verificar valores nulos y duplicados
...
# Cell 6: Separar datos en variables categóricas y numéricas
variables_categoricas = ['sex', 'year', 'glang', 'part', 'job', 'stud_h', 'health', 'psyt']
variables_numericas = ['age', 'jspe', 'qcae_cog', 'qcae_aff', 'amsp', 'erec_mean']
...
# Cell 7: Descripción básica de datos categóricos
print("Descripción de datos categóricos:")
print(df_cat.describe(include='all'))

# Cell 8: Contar categorías en cada variable categórica
for col in df_cat.columns:
    print(f"\nDistribución de {col}:")
    print(pd.crosstab(index=df_cat[col], columns='count'))
    print(pd.crosstab(index=df_cat[col], columns='percentage', normalize=True))

# Cell 9: Gráficas de variables categóricas
plt.figure(figsize=(20, 20))
for i, col in enumerate(df_cat.columns):
    plt.subplot(3, 3, i + 1)
    sns.countplot(x=col, data=df_cat)
...
# Cell 10: Prueba Chi-Square entre variables categóricas
chi2_table = []
for i, col1 in enumerate(df_cat.columns):
    for j, col2 in enumerate(df_cat.columns):
        if i < j:
...
print("Resultados Chi-Square:")
print(chi2_table)

# Cell 11: Relación entre variables categóricas
for i, row in chi2_table.iterrows():
...
# Cell 12: Procesamiento de variables numéricas
...
# Cell 14: PCA (Reducción de dimensionalidad)
scaler = StandardScaler()
df_num_scaled = scaler.fit_transform(df_num)
...
silhouette_avg = silhouette_score(df_num_scaled, clusters)
print(f"Silhouette Score: {silhouette_avg}")

# Cell 18: Regresión logística
...
# Cell 19: Distribución por cluster
df['Cluster'] = clusters
for col in variables_categoricas:
    cross_tab = pd.crosstab(df[col], df['Cluster'], normalize='columns')
